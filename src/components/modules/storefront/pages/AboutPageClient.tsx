...
import { StorefrontCTA, StorefrontReveal } from "@/components/modules/storefront/ui";
...
export default function AboutPageClient() {
  ...
  return (
    <>
      <StorePageTitle pageId="about" brandName={store.brandName} />
...
function AboutPageContent({ store }: { store: Store }) {
  ...
  return (
    <main className="sf-section w-full py-12 md:py-16">
      ...
      <StorefrontReveal>
        <div className="sf-border sf-promo-panel sf-panel mb-12 overflow-hidden border">
          <div className="grid items-stretch lg:grid-cols-2">
            {hasImage ? (
              <div className="sf-image-zoom relative min-h-[280px] overflow-hidden lg:min-h-[400px]">
                <Image
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:gap-8">
        {page.sections.map((section, index) => (
          <StorefrontReveal key={section.title} staggerIndex={index}>
            <article className="sf-editorial-card sf-card-interactive h-full p-6 md:p-8">
              <div className="mb-4 flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[var(--sf-muted)] transition-transform duration-300 group-hover:scale-105">
                  <Sparkles className="h-4 w-4 sf-accent-text" />
                </div>
